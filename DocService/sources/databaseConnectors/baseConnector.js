...
const reservoirMaximum =
  cfgBottleneckGetChanges.reservoirIncreaseMaximum ||
  cfgBottleneckGetChanges.reservoirRefreshAmount;
...
function getChangesSize(changes) {
  return changes.reduce(
    (accumulator, currentValue) => accumulator + currentValue.change_data.length,
    0
  );
}
...
function insertChangesPromiseFast(ctx, objChanges, docId, index, user) {
  return new Promise(function (resolve, reject) {
    dbInstance.insertChanges(
      ...
      index,
      user,
      function (error, result, isSupported) {
...
function _deleteChanges(ctx, docId, deleteIndex) {
  deleteChangesCallback(ctx, docId, deleteIndex, function () {
...
function unLockCriticalSection(id) {
  ...
  arrCallbacks.shift();
  if (0 < arrCallbacks.length) arrCallbacks[0]();
  else delete g_oCriticalSection[id];
}
