...
function* getConvertStatus(ctx, docId, encryptedUserPassword, selectRes, opt_checkPassword) {
  ...
  if (selectRes.length > 0) {
    var row = selectRes[0];
    let password =
      opt_checkPassword && sqlBase.DocumentPassword.prototype.getCurPassword(ctx, row.password);
    switch (row.status) {
...
function* getConvertPath(ctx, docId, fileTo, formatTo) {
  if (
    constants.AVS_OFFICESTUDIO_FILE_OTHER_OOXML === formatTo ||
...
function* getConvertUrl(ctx, baseUrl, fileToPath, title) {
  ...
  return yield storage.getSignedUrl(
    ctx,
    baseUrl,
    fileToPath,
...
async function convertFromChanges(
  ...
  let commandSfctByCmdRes = await canvasService.commandSfctByCmd(
    ctx,
    cmd,
    opt_priority,
    opt_expiration,
  ...
  let status = await co(
    convertByCmd(ctx, cmd, true, fileTo, undefined, opt_priority, opt_expiration, opt_queue)
  );
  if (status.end) {
    ...
    try {
      ...
      if (params.key && !constants.DOC_ID_REGEX.test(params.key)) {
        ...
        utils.fillResponse(
          req,
          res,
          new commonDefines.ConvertStatus(constants.CONVERT_PARAMS),
          isJson
        );
        return;
      }
      ...
      if (params.pdf) {
        if (
          true === params.pdf.pdfa &&
          constants.AVS_OFFICESTUDIO_FILE_CROSSPLATFORM_PDF === outputFormat
        ) {
          outputFormat = constants.AVS_OFFICESTUDIO_FILE_CROSSPLATFORM_PDFA;
        } else if (
          false === params.pdf.pdfa &&
      ...
      cmd.setCodepage(
        commonDefines.c_oAscEncodingsMap[params.codePage] || commonDefines.c_oAscCodePageUtf8
      );
      ...
      if (params.password) {
        if (params.password.length > constants.PASSWORD_MAX_LENGTH) {
          ctx.logger.warn(
            "convertRequest password too long actual = %s; max = %s",
      ...
      if (thumbnail) {
        ...
        if (
          false === thumbnailData.getFirst() &&
          0 !== (constants.AVS_OFFICESTUDIO_FILE_IMAGE & cmd.getOutputFormat())
        ) {
          outputExt = "zip";
        }
      }
      ...
      if (constants.AVS_OFFICESTUDIO_FILE_UNKNOWN !== cmd.getOutputFormat()) {
        ...
        ctx.logger.warn(
          "Error convert unknown outputtype: query = %j from = %s",
          params,
          addresses
        );
  ...
  return co(function* () {
    ...
    try {
      ...
      if (error === constants.NO_ERROR && (params.key || params.url || isInBody)) {
        ...
        error = status.err;
        if (end) {
          urls = yield storageBase.getSignedUrls(
            ctx,
            utils.getBaseUrlByRequest(ctx, req),
            docId + "/output",
            commonDefines.c_oAscUrlTypes.Temporary
      ...
      ctx.logger.debug(
        "End builderRequest request: urls = %j end = %s error = %s",
        urls,
        end,
        error
      );
    ...
    try {
      ...
      if (password) {
        if (password.length > constants.PASSWORD_MAX_LENGTH) {
          ctx.logger.warn(
            "convert-to Password too long actual = %s; max = %s",
            password.length,
            constants.PASSWORD_MAX_LENGTH
          );
          res.sendStatus(400);
      ...
      if (passwordToOpen) {
        if (passwordToOpen.length > constants.PASSWORD_MAX_LENGTH) {
          ctx.logger.warn(
            "convert-to PasswordToOpen too long actual = %s; max = %s",
            passwordToOpen.length,
            constants.PASSWORD_MAX_LENGTH
          );
          res.sendStatus(400);
          return;
      ...
      if (status && status.end && constants.NO_ERROR === status.err) {
        let filename =
          path.basename(originalname, path.extname(originalname)) + path.extname(fileTo);
