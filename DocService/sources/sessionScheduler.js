...
class SessionScheduler {
  ...
  configureConnection(connId, options) {
    ...
    if (!state || state.removed) return;
    const now = Date.now();
    if (Object.prototype.hasOwnProperty.call(options, 'idleMs')) {
    ...
    if (Object.prototype.hasOwnProperty.call(options, 'absoluteMs')) {
  ...
  async _onTick() {
    ...
    while (top && top.time <= now) {
      ...
      if (item.type === 'idle') {
        // Skip stale
        if (item.version !== state.idleVersion) {
          top = this._heap._peek();
          continue;
        }
        await this._processIdle(state, now);
      } else if (item.type === 'absolute') {
        if (item.version !== state.absoluteVersion) {
          top = this._heap._peek();
          continue;
        }
  ...
  async _processIdle(state, now) {
    ...
    if (!state.idleWarned) {
      ...
      try {
        await this._callbacks.onIdleWarning(state.conn, idleMs);
      } catch (_e) {
        void _e;
      }
    ...
    try {
      await this._callbacks.onIdleClose(state.conn);
    } catch (_e) {
      void _e;
    }
  ...
  async _processAbsolute(state, now) {
    ...
    if (!state.absoluteWarned) {
      ...
      try {
        await this._callbacks.onAbsoluteWarning(state.conn);
      } catch (_e) {
        void _e;
      }
    ...
    try {
      await this._callbacks.onAbsoluteClose(state.conn);
    } catch (_e) {
      void _e;
    }
    this.removeConnection(state.connId);
