...
function runTestForDir(ctx, isMultitenantMode, specialDir) {
  ...
  if ("storage-fs" === getStorageCfg(specialDir).name) {
    ...
    test.skip("uploadObject - stream error handling", async () => {
      ...
      await expect(
        storage.uploadObject(ctx, "test-error-file.txt", "nonexistent.txt", specialDir)
    ...
    test.skip("uploadObject - non-existent file handling", async () => {
      ...
      await expect(
        storage.uploadObject(ctx, "test-error-file.txt", nonExistentFile, specialDir)
  ...
  test("getSignedUrls", async () => {
    ...
    expect(data.sort()).toEqual(
      [testFileData1, testFileData2, testFileData3, testFileData4].sort()
    );
  });
  test("getSignedUrlsArrayByArray", async () => {
    let urls, data;
    urls = await storage.getSignedUrlsArrayByArray(
      ctx,
      baseUrl,
      [testFile1, testFile2],
      urlType,
  ...
  test("getSignedUrlsByArray", async () => {
    let urls, data;
    urls = await storage.getSignedUrlsByArray(
      ctx,
      baseUrl,
      [testFile3, testFile4],
      undefined,
