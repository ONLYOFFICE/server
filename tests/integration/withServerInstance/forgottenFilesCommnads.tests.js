  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (cfgTokenEnableRequestOutbox) {
      ...
      const token = await signToken(
        ...
        constants.c_oAscSecretType.Inbox,
        secret
      );
...
beforeAll(async function () {
  ...
  for (const index in testFilesNames) {
    await storage.putObject(
      ctx,
      `${testFilesNames[index]}/${cfgForgottenFilesName}.docx`,
      buffer,
      buffer.length,
      cfgForgottenFiles
    );
  ...
  describe("Forgotten files commands verification", function () {
      ...
      const createExpected = ({key, error}) => {
        ...
        if ("storage-fs" === cfgStorageName || !cfgUseDirectStorageUrls) {
          if ("storage-fs" === cfgStorageName) {
            urlPattern =
              "http://localhost:8000/cache/files/forgotten/--key--/output.docx/output.docx";
          } else {
            urlPattern =
              "http://localhost:8000/storage-cache/files/forgotten/--key--/output.docx/output.docx";
          }
        } else if ("storage-s3" === cfgStorageName) {
          let host =
            cfgEndpoint.slice(0, "https://".length) +
            cfgBucketName +
            "." +
            cfgEndpoint.slice("https://".length);
    ...
    describe("deleteForgotten", function () {
      ...
      for (const testCase in testCases) {
        test(testCase, async () => {
          ...
          const alreadyExistedDirectories = getKeysDirectories(
            await storage.listObjects(ctx, "", cfgForgottenFiles)
          );
          const directoryToBeDeleted =
            testCases[testCase].error !== 0 ? "--not-existed--" : testCases[testCase].key;
          const shouldExist = alreadyExistedDirectories.filter(
            (directory) => directoryToBeDeleted !== directory
          );
          ...
          const directoriesExistedAfterDeletion = getKeysDirectories(
            await storage.listObjects(ctx, "", cfgForgottenFiles)
          );
          expect(actual).toEqual(expected);
    ...
    describe("getForgottenList", function () {
      test("Main case", async () => {
        ...
        const docId =
          "DocService-DocsCoServer-forgottenFilesCommands-getForgottenList-2-integration-test";
        const buffer = Buffer.from("getForgottenList test file");
        await storage.putObject(
          ctx,
          `${docId}/${cfgForgottenFilesName}.docx`,
          buffer,
          buffer.length,
          cfgForgottenFiles
        );
