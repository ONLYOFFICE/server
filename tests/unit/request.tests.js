...
describe("HTTP Request Unit Tests", () => {
  ...
  describe("specific timeout behaviors", () => {
    test.concurrent(
      "connectionAndInactivity triggers when server delays response headers",
      async () => {
        try {
    ...
    test.concurrent(
      "connectionAndInactivity does not trigger when longer than server delay",
      async () => {
    ...
    test.concurrent(
      "wholeCycle triggers even when server starts sending data but does not complete",
      async () => {
        try {
          await utils.downloadUrlPromise(
            ctx,
    ...
    test.concurrent(
      "connectionAndInactivity triggers when server stops sending data midway",
      async () => {
        try {
          await utils.downloadUrlPromise(
            ctx,
    ...
    test.concurrent(
      "connectionAndInactivity does not trigger when longer than inactivity periods",
      async () => {
        const result = await utils.downloadUrlPromise(
          ctx,
          `${BASE_URL}/api/slow-body`,
    ...
    test.concurrent(
      "wholeCycle does not trigger when longer than total response time",
      async () => {
        const result = await utils.downloadUrlPromise(
          ctx,
          `${BASE_URL}/api/slow-body`,
  ...
  describe("downloadUrlPromise", () => {
    ...
    test.concurrent("disables compression when gzip is false", async () => {
      ...
      expect(
        responseBody.headers?.["accept-encoding"] === "identity" ||
          responseBody.headers?.["accept-encoding"] === undefined
      ).toBe(true);
    });
    ...
    test.concurrent("successfully routes GET request through a real proxy", async () => {
      try {
        ...
        const proxyRequest = proxiedRequests.find(
          (r) => r.method === "GET" && r.url.includes("/api/data")
        );
  ...
  describe("postRequestPromise", () => {
    ...
    test.concurrent("successfully routes POST request through a real proxy", async () => {
      try {
        ...
        const postProxyRequest = proxiedRequests.find(
          (r) => r.method === "POST" && r.url.includes("/api/post")
        );
