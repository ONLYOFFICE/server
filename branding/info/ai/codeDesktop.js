...
Available functions:\n";
  ...
  function registerActions() {
    ...
    AI.Actions[AI.ActionType.ImageGeneration] = new ActionUI(
      "Image generation",
      "image-ai",
      "",
      AI.CapabilitiesUI.Image
    );
    AI.Actions[AI.ActionType.OCR] = new ActionUI(
      "OCR",
      "text-analysis-ai",
      "",
      AI.CapabilitiesUI.Vision
    );
    AI.Actions[AI.ActionType.Vision] = new ActionUI(
      "Vision",
      "vision-ai",
      "",
      AI.CapabilitiesUI.Vision
  ...
  function showLoader() {
    document.getElementById("loaderId").style.marginLeft = getComputedStyle(
      document.body
    ).marginLeft;
  ...
  window.Asc.Editor.callMethod = function (method, params) {
    if (method === "StartAction") showLoader();
    else if (method === "EndAction") hideLoader();
  };
  ...
  function trimResult(data, posStart, isSpaces, extraCharacters) {
    ...
    if (-1 != pos) {
      let trimC = ['"', "'", "\n", "\r", "`"];
      if (extraCharacters) trimC = trimC.concat(extraCharacters);
  ...
  function initializeCode() {
    ...
    async function GO() {
      ...
      if (0 === result.indexOf("[functionCalling (")) {
        ...
        if (result === "") {
          shownResult =
            "System tool wasn't foundSystem tool not found. Please clarify your question.";
        } else {
    ...
    try {
      ...
      if (!lang) lang = "en";
      if (2 == lang.length) lang = lang + "-" + lang.toUpperCase();
      ...
      if (isFound) {
        ...
        if (window.Asc.plugin.onTranslate) window.Asc.plugin.onTranslate();
      }
    } catch (e) {
  ...
  window.Asc.plugin.onTranslate = function () {
    ...
    document.getElementById("chatInput").placeholder =
      window.Asc.plugin.tr("Enter message") + "...";
