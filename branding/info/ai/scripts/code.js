...
window.addSupportAgentMode = function () {
  ...
  window.Asc.plugin.attachEditorEvent("onKeyDown", function (e) {
    ...
    if (e.keyCode === codeShow && isCtrl && !helperWindow) {
      if (isClearHistory) agentHistory = [];
      ...
      helperWindow.attachEvent("onHelperAction", async function (prompt) {
        ...
        async function onStreamEvent(data, end) {
          if (isSupportStreaming) await Asc.Library.PasteText(data);
          dataStream += data;
        ...
        let result = await requestEngine.chatRequest(copyMessages, false, async function (data) {
          ...
          if (isSupportStreaming) await checkEndAction();
          ...
          if (!checkBuffer) await onStreamEvent(data);
        });
        ...
        if (checkBuffer) {
          if (agentDebug) console.log(buffer);
          ...
          if (resultFunc) {
            ...
            if (resultFunc.prompt !== undefined) {
              agentHistory.push({
                role: "user",
                content: resultFunc.prompt,
              });
            }
          }
        } else {
          if (agentDebug) console.log(result);
      ...
      setTimeout(function () {
        if (helperWindow) helperWindow.isSkipClick = false;
      }, 500);
    }
  });
  ...
  if (3 === initCounter) {
    ...
    if (Asc.Editor.getType() === "pdf") {
      window.Asc.plugin.attachEditorEvent("onChangeRestrictions", function (value) {
        ...
        if (window.buttonOCRPage.disabled !== disabled) window.buttonOCRPage.disabled = disabled;
        Asc.Buttons.updateToolbarMenu(window.buttonMainToolbar.id, window.buttonMainToolbar.name, [
          window.buttonOCRPage,
        ]);
      });

      let restriction = Asc.plugin.info.restrictions;
      if (undefined === restriction) restriction = 0;
      ...
      if (0x80 & restriction) buttonOCRPage.disabled = true;
      ...
      buttonOCRPage.attachOnClick(async function (data) {
        ...
        let content = await Asc.Editor.callMethod("GetPageImage", [
          pageIndex,
          {
            maxSize: 1024,
            annotations: true,
        ...
        await Asc.Editor.callMethod("ReplacePageContent", [
          pageIndex,
          {
            type: "html",
            options: {
	...
	{\n\
        ...
        if (isUpdate) await Asc.Editor.callMethod("SetCustomFunctions", [JSON.stringify(oldCF)]);
      }
    }

    if (editorVersion >= 9000004) window.addSupportAgentMode();
  }
}
...
function updateActions() {
  if (settingsWindow) settingsWindow.command("onUpdateActions", AI.ActionsGetSorted());
}
...
function onOpenAiModelsModal() {
  ...
  let variation = {
    ...
    isVisual: true,
    buttons: [{text: window.Asc.plugin.tr("OK"), primary: false}],
    isModal: true,
    EditorsSupport: ["word", "slide", "cell", "pdf"],
    size: [320, 230],
  };
...
function onOpenSummarizationModal() {
  ...
  summarizationWindow.attachEvent("onSummarize", async function (data) {
    switch (data.type) {
      case "review": {
        if (Asc.plugin.info.editorType === "word") await Asc.Library.InsertAsReview(data.data);
