...
function registerButtons(window, undefined) {
  ...
  if (true) {
    ...
    button5.attachOnClick(async function () {
      ...
      let content = await Asc.Library.GetSelectedText();
      if (!content) content = await Asc.Library.GetCurrentWord();
  ...
  const on_click_text_to_image = async function (params) {
    ...
    if (Asc.plugin.info.editorSubType === "pdf") return await Asc.Library.AddGeneratedImage(result);
  ...
  if (true) {
    ...
    if (!AI.serverSettings) button1.separator = true;
