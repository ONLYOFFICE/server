...
class Provider extends AI.Provider {
  ...
  checkModelCapability = function (model) {
    ...
    if (model.capabilities && model.capabilities.vision) capUI = AI.CapabilitiesUI.Vision;
    return capUI;
  };
  ...
  getEndpointUrl(endpoint, model) {
    ...
    if (!url) return super.getEndpointUrl(endpoint, model);
    return url;
  }
  ...
  getImageOCRResult(messageInput, model) {
    ...
    let images = [];
    let markdownContent = "";
    if (!message.pages) return markdownContent;
