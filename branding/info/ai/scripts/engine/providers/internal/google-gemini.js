...
class Provider extends AI.Provider {
  ...
  correctModelInfo(model) {
    ...
    if (index === 0) model.name = model.name.substring(7);
  }

  checkExcludeModel(model) {
    if (model.id === "models/chat-bison-001" || model.id === "models/text-bison-001") return true;

    if (-1 !== model.id.indexOf("gemini-1.0")) return true;

    return false;
  }

  checkModelCapability(model) {
    if (model.inputTokenLimit) model.options.max_input_tokens = model.inputTokenLimit;
    ...
    if (
      ...
      if (-1 !== model.id.indexOf("vision")) caps |= AI.CapabilitiesUI.Vision;
    ...
    switch (endpoint) {
      ...
      default:
        ...
        if (endpoint === Types.v1.Images_Generations) {
          if (-1 != model.id.indexOf("imagen-")) addon = ":predict";
        }
        url = "/" + model.id + addon;
        break;
    }
  ...
  getChatCompletions(message, model) {
    ...
    for (let i = 0, len = message.messages.length; i < len; i++) {
      ...
      if (rec.role === "assistant") rec.role = "model";
