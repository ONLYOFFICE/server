...
(function () {
  ...
  let keys = [];
  for (let i in AI.InputMaxTokens) keys.push(i);
  ...
  for (let item in AI.CapabilitiesUI) capabilitiesAll |= AI.CapabilitiesUI[item];
  ...
  AI.createProviderInstance = function (name, url, key, addon) {
    for (let i = 0, len = window.AI.InternalCustomProviders.length; i < len; i++) {
      if (name === AI.InternalCustomProviders[i].name)
        return AI.InternalCustomProviders[i].createInstance(
          name,
          url,
          key,
          addon || AI.InternalCustomProviders[i].addon
        );
    }
    for (let i = 0, len = window.AI.InternalProviders.length; i < len; i++) {
      if (name === AI.InternalProviders[i].name)
        return AI.InternalProviders[i].createInstance(
          name,
          url,
          key,
          addon || AI.InternalProviders[i].addon
        );
    }
  ...
  AI.isInternalProvider = function (name) {
    for (let i = 0, len = AI.InternalProviders.length; i < len; i++) {
      if (name === AI.InternalProviders[i].name) return true;
    }
    return false;
  };
  ...
  AI.loadInternalProviders = async function () {
    ...
    if ("" === providersText) return;

    try {
      let providers = JSON.parse(providersText);
      for (let i = 0, len = providers.length; i < len; i++) {
        let providerContent = await AI.loadResourceAsText(
          "./scripts/engine/providers/internal/" + providers[i] + ".js"
        );
        if (providerContent !== "") {
  ...
  AI.removeCustomProvider = function (name) {
    if (AI.InternalCustomProvidersSources[name]) delete AI.InternalCustomProvidersSources[name];
  ...
  AI.getCustomProviders = function () {
    ...
    for (let i in AI.Providers) {
      ...
      if (
        internalProvider &&
        (internalProvider.url !== provider.url || internalProvider.addon !== provider.addon)
      ) {
