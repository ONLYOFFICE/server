...
(async function () {
  class Provider {
    ...
    getChatCompletionsResult(message, model) {
      ...
      if (!arrResult) return result;

      let choice = arrResult[0] ? arrResult[0] : arrResult;
      if (!choice) return result;

      if (choice.message && choice.message.content) result.content.push(choice.message.content);
      if (choice.text) result.content.push(choice.text);
      if (choice.content) {
        if (typeof choice.content === "string") result.content.push(choice.content);
      ...
      if (choice.delta && choice.delta.content) result.content.push(choice.delta.content);
      if (choice.delta && choice.delta.text) result.content.push(choice.delta.text);

      let trimArray = ["\n".charCodeAt(0)];
      for (let i = 0, len = result.content.length; i < len; i++) {
        let iEnd = result.content[i].length - 1;
        let iStart = 0;
        while (iStart < iEnd && trimArray.includes(result.content[i].charCodeAt(iStart))) iStart++;
        while (iEnd > iStart && trimArray.includes(result.content[i].charCodeAt(iEnd))) iEnd--;
    ...
    async getImageGenerationResult(message, model) {
      ...
      let getProp = function (name) {
        ...
        if (message.data && message.data[name]) return message.data[name];
        return undefined;
      };

      if (!imageUrl) {
        let data = getProp("data");
        if (data && data[0] && data[0].b64_json) imageUrl = data[0].b64_json;
      }

      if (!imageUrl) {
        let artifacts = getProp("artifacts");
        if (artifacts && artifacts[0] && artifacts[0].base64) imageUrl = artifacts[0].base64;
      }

      if (!imageUrl) {
        let result = getProp("result");
        if (result && result.imageUrl) imageUrl = result.imageUrl;
      }

      if (!imageUrl) {
        let generations = getProp("generations");
        if (generations && generations[0] && generations[0].url) imageUrl = generations[0].url;
      }

      if (!imageUrl) {
        let candidates = getProp("candidates");
        if (candidates && candidates[0] && candidates[0].content) imageUrl = candidates[0].content;
      }
      ...
      if (!imageUrl) {
        ...
        if (response) {
          ...
          if (matches && matches[1]) imageUrl = matches[1];
        }
      }
