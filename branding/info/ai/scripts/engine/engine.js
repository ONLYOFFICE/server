  ...
  window.fetch = function (url, obj) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      for (let h in obj.headers)
        if (obj.headers.hasOwnProperty(h)) xhr.setRequestHeader(h, obj.headers[h]);

      xhr.onload = function () {
        if (this.status == 200 || this.status == 0) resolve(new TextResponse(this.response, true));
...
(function (window, undefined) {
  async function requestWrapper(message) {
    return new Promise(function (resolve, reject) {
      if (
        AI.isLocalDesktopForNotStreamedRequests &&
  ...
  AI._getEndpointUrl = function (_provider, endpoint, model, options) {
    ...
    if (!_provider.createInstance && _provider.url !== undefined) url = _provider.url;
    ...
    if ("" !== model.provider) {
      ...
      if (provider) {
        for (let i = 0, len = provider.models.length; i < len; i++) {
          if (model.id === provider.models[i].id || model.id === provider.models[i].name) {
  ...
  AI.Request.prototype._chatRequest = async function (content, streamFunc) {
    ...
    if (this.model) {
      ...
      if (isFoundCompletions && !isFoundChatCompletions) isUseCompletionsInsteadChat = true;
    }
    ...
    if (max_input_tokens < header_footer_overhead) max_input_tokens = header_footer_overhead + 1000;
    ...
    if (input_tokens < max_input_tokens || isNoSplit) {
      ...
      let chunkLen =
        (((max_input_tokens - header_footer_overhead) / input_tokens) * input_len) >> 0;
      let currentLen = 0;
    ...
    let processResult = function (data) {
      ...
      if (0 === result.content[0].indexOf("<think>")) {
        ...
        if (end !== -1) result.content[0] = result.content[0].substring(end + 8);
      }
    ...
    if (1 === messages.length) {
      if (!isUseCompletionsInsteadChat) {
        if (isMessages) requestBody.messages = messages[0];
        else requestBody.messages = [{role: "user", content: messages[0]}];

        objRequest.body = provider.getChatCompletions(requestBody, this.model);

        if (isStreaming && options.streamingBody !== false) objRequest.body.stream = true;
      } else {
      ...
      if (isStreaming) readerAsync = await requestWrapperStream(objRequest);
      ...
      if (!readerAsync) {
        ...
        while (true) {
          ...
          if (streamFunc) await streamFunc(dataChunk);
        }
        return allChunks;
      }
    } else {
      ...
      function getFooter(part, partsCount) {
        ...
        if (part != partsCount) {
          ...
          footer +=
            "ALL PARTS SENT. Now you can continue processing the request." + lastFooterForOldModels;
        }
        return footer;
      }
  ...
  AI.Request.prototype._imageGenerationRequest = async function (data) {
    ...
    if (objRequest.body instanceof FormData) objRequest.isBlob = true;
