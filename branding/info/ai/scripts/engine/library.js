...
(function (exports, undefined) {
  ...
  Editor.getType = function () {
    if (Asc.plugin.info.editorSubType === "pdf") return "pdf";
  ...
  Library.prototype.ConvertMdToHTML = function (data, plugins) {
    ...
    if (plugins) {
      for (let i = 0, len = plugins.length; i < len; i++) c.use(plugins[i]);
    }
  ...
  Library.prototype.getMarkdownResult = function (data) {
    ...
    if (-1 !== markdownEscape && markdownEscape < 5) data = data.substring(markdownEscape + 5);
