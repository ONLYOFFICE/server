...
window.Asc.plugin.init = function () {
  ...
  window.Asc.plugin.attachEvent("onSetCustomProvider", function (list) {
    let items = [];
    for (let i = 0, len = list.length; i < len; i++) items.push({name: list[i]});
...
window.Asc.plugin.onTranslate = function () {
  ...
  new Tooltip(document.getElementById("alert-icon"), {
    renderInner: function () {
      ...
      textEl.innerText = window.Asc.plugin.tr(
        "Enter the configuration for the AI model API in JS format. Provide the model name, endpoint URLs, and headers."
      );
