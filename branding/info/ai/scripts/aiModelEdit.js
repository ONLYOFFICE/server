...
function getModelById(id) {
  for (let i = 0, len = modelsList.length; i < len; i++) {
    if (modelsList[i].id === id) return modelsList[i];
  }
  return null;
}
...
function onChangeProviderComboBox() {
  var provider =
    providersList.filter(function (el) {
      return el.id == providerNameCmbEl.value;
    })[0] || null;
...
function onOpenProviderComboBox() {
  const searchField = $(providerNameCmbEl)
    .data("select2")
...
function onChangeModelComboBox() {
  var modelObj =
    providerModelsList.filter(function (model) {
      return model.name == modelNameCmbEl.value;
    })[0] || null;
  ...
  if (modelObj && modelObj.name) {
    let providerObj =
      providersList.filter(function (provider) {
        return provider.id == providerNameCmbEl.value;
      })[0] || null;
    let providerName = providerObj ? providerObj.name : providerNameCmbEl.value;
    if (providerName) nameInputEl.value = providerName + " [" + modelObj.name + "]";
    else nameInputEl.value = modelObj.name;
  }
}
...
function updateModelsList() {
  ...
  var startLoader = function () {
    updateModelsLoader &&
      (updateModelsLoader.remove
        ? updateModelsLoader.remove()
        : $("#update-models-loader-container")[0].removeChild(updateModelsLoader));
    updateModelsLoader = showLoader(
      $("#update-models-loader-container")[0],
      window.Asc.plugin.tr("Updating")
    );
