...
(function (a, t) {
  ...
  a.Asc.plugin.tr = function (c) {
    return c;
  };
  a.Asc.scope = {};
  a.Asc.scope.prototype = {
    clear: function () {
      for (var c in a.Asc.scope) delete a.Asc.scope[c];
    },
  };
  a.onload = function () {
    if (a.Asc && a.Asc.plugin) {
        ...
        if (a.Asc && a.Asc.plugin) {
          ...
          if (200 == c.status || (0 == c.status && 4 == c.readyState)) {
            ...
            g &&
              ...
              (g.ondragenter = function (k) {
                k && k.preventDefault && k.preventDefault();
                return !1;
              }),
              (g.ondragover = function (k) {
                k && k.preventDefault && k.preventDefault();
                k && k.dataTransfer && (k.dataTransfer.dropEffect = "none");
                return !1;
              }));
      ...
      c.onerror = function () {
        if (a.Asc && a.Asc.plugin && m()) {
          var d = {type: "initialize", guid: a.Asc.plugin.guid};
  ...
  a.Asc.plugin._onCustomMenuClick = function (c, d) {
    var g = t,
      k = d.indexOf("_oo_sep_");
    -1 !== k && ((g = d.substring(k + 8)), (d = d.substring(0, k)));
    this[c] && this[c][d] && this[c][d].call(this, g);
  };
  ...
  a.Asc.plugin.attachEvent = function (c, d) {
    var g = a.Asc.plugin;
    g._events || (g._events = {});
    g._events[c] = d;
  };
  a.Asc.plugin.detachEvent = function (c) {
    var d = a.Asc.plugin;
    d._events && d._events[c] && delete d._events[c];
  };
  a.Asc.plugin.onEvent = function (c, d) {
    var g = a.Asc.plugin;
    g._events && g._events[c] && g._events[c].call(g, d);
  };
  a.Asc.plugin.attachEditorEvent = function (c, d) {
    a.Asc.plugin["event_" + c] = d.bind(a.Asc.plugin);
    v(JSON.stringify({guid: a.Asc.plugin.guid, type: "attachEvent", name: c}));
  };
  a.Asc.plugin.detachEditorEvent = function (c) {
...
(function (a, t) {
  ...
  d.Buttons.registerContextMenu = function () {
    a.Asc.plugin.attachEvent("onContextMenuShow", function (b) {
      if (b) {
        var e = {guid: a.Asc.plugin.guid};
        for (let h = 0, f = d.Buttons.ButtonsContextMenu.length; h < f; h++) {
          let l = d.Buttons.ButtonsContextMenu[h];
  ...
  d.Buttons.registerToolbarMenu = function () {
    let b = {guid: a.Asc.plugin.guid, tabs: []};
    for (let e = 0, h = d.Buttons.ButtonsToolbar.length; e < h; e++) {
      let f = d.Buttons.ButtonsToolbar[e];
      null === f.parent && f.toToolbar(b);
      if (f.menu)
        for (let l in f.menu) {
          let n = f.menu.hasOwnProperty(l) ? f.menu[l] : null;
          n && n.onclick && a.Asc.plugin.attachToolbarMenuClickEvent(n.id, n.onclick);
        }
    }
    0 < b.tabs.length && a.Asc.plugin.executeMethod("AddToolbarMenuItem", [b]);
  };
  d.Buttons.updateToolbarMenu = function (b, e, h) {
  ...
  d.Buttons.registerContentControl = function () {
    a.Asc.plugin.attachEditorEvent("onShowContentControlTrack", function (e) {
      let h = {guid: a.Asc.plugin.guid, items: {}},
        f = [];
    ...
    let b = a.Asc.plugin;
    a.Asc.plugin.attachEditorEvent("onContentControlButtonClick", function (e) {
      let h = e && e.buttonId ? e.buttonId : null;
      e = e && e.contentControlId ? e.contentControlId : null;
      h &&
        e &&
  ...
  var g = {Button: "button", BigButton: "big-button"},
    k = {None: 0, ContextMenu: 1, Toolbar: 2, ContentControl: 3};
  p.prototype.toItem = function () {
    let b = {id: this.id, text: r(this.text)};
    ...
    this.menu &&
      (b.items = this.menu.map(function (e) {
        e.text = r(e.text);
        return e;
      }));
  ...
  p.prototype.attachOnClick = function (b) {};
  ...
  m.prototype.copy = function () {
    let b = new m(this.parent, this.id);
  ...
  m.prototype.onContextMenuShowAnalyze = function (b, e) {
    return !1;
  };
  m.prototype.onContextMenuShowExtendItem = function (b, e) {};
  ...
  q.prototype.toToolbar = function (b) {
    if (null === this.parent) {
      var e = {id: this.id, text: r(this.text), items: []};
      null !== this.hint && (e.hint = r("" === this.hint ? this.hint : this.text));
      b.tabs.push(e);
    } else ((e = this.toItem()), b.items || (b.items = []), b.items.push(e));
    if (this.childs)
      for (let h = 0, f = this.childs.length; h < f; h++) this.childs[h].toToolbar(e);
  };
