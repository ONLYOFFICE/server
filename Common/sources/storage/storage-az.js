...
async function listObjectsExec(storageCfg, prefix, output = []) {
  ...
  const finalOptions = applyCommandOptions(listOptions, storageCfg, 'listBlobsFlat');

  for await (const blob of containerClient.listBlobsFlat(finalOptions)) {
...
async function putObject(storageCfg, strPath, buffer, contentLength) {
  ...
  if (buffer instanceof Buffer) {
    ...
    const finalOptions = applyCommandOptions(uploadOptions, storageCfg, 'uploadData');
    await blobClient.uploadData(buffer, finalOptions);
    ...
    await blobClient.uploadStream(buffer, undefined, undefined, finalOptions);
  } else {
