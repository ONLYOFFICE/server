...
TaskQueueDataConvert.prototype = {
  ...
  serializeOptions: function (ctx, isInJwtToken, oformAsPdf) {
    ...
    let allowPrivateIP =
      !tenExternalRequestAction.blockPrivateIP && tenRequesFilteringAgent.allowPrivateIPAddress;
...
function* downloadFile(ctx, uri, fileFrom, withAuthorization, isInJwtToken, opt_headers) {
  const tenMaxDownloadBytes = ctx.getCfg(
  ...
  const tenDownloadAttemptMaxCount = ctx.getCfg(
    "FileConverter.converter.downloadAttemptMaxCount",
    cfgDownloadAttemptMaxCount
  );
  const tenDownloadAttemptDelay = ctx.getCfg(
    "FileConverter.converter.downloadAttemptDelay",
    cfgDownloadAttemptDelay
  );
  ...
  if (0 == filterStatus) {
    while (constants.NO_ERROR !== res && downloadAttemptCount++ < tenDownloadAttemptMaxCount) {
      try {
        ...
        ctx.logger.error(
          "error downloadFile:url=%s;attempt=%d;code:%s;connect:%s %s",
          uri,
...
function* processDownloadFromStorage(ctx, dataConvert, cmd, task, tempDirs, authorProps) {
  ...
  if (task.getFromOrigin() || task.getFromSettings()) {
    if (task.getFromChanges()) {
      ...
      let filesCount = 0;
      if (cmd.getSaveKey()) {
        filesCount = yield* downloadFileFromStorage(
          ctx,
  ...
  if (mailMergeSend) {
    yield* downloadFileFromStorage(
      ctx,
      cmd.getDocId() + mailMergeSend.getJsonKey(),
      tempDirs.source
    );
    concatDir = tempDirs.source;
  }
...
function* processChangesBin(ctx, tempDirs, task, cmd, authorProps, sha256) {
  const tenStreamWriterBufferSize = ctx.getCfg(
    "FileConverter.converter.streamWriterBufferSize",
    cfgStreamWriterBufferSize
  );
  ...
  let streamObj = yield* streamCreateBin(ctx, changesDir, indexFile++, {
    highWaterMark: tenStreamWriterBufferSize,
  });
  ...
  while (curIndexStart < curIndexEnd || extChanges) {
    ...
    if (curIndexStart < curIndexEnd) {
      changes = yield baseConnector.getChangesPromise(
        ctx,
        cmd.getDocId(),
        curIndexStart,
        curIndexEnd,
      ...
      if (
        changes.length > 0 &&
        changes[0].change_data.subarray(0, "ENCRYPTED;".length).includes("ENCRYPTED;")
      ) {
    ...
    for (let i = 0; i < changes.length; ++i) {
      ...
      if (null === changesAuthor || changesAuthor !== change.user_id_original) {
        ...
        changesHistory.changes.push({
          ...
          created: strDate,
          user: {id: change.user_id_original, name: change.user_name},
        });
      }
    ...
    if (changes.length > 0) {
      ...
      authorProps.modified =
        changes[changes.length - 1].change_date.toISOString().slice(0, 19) + "Z";
    }
  ...
  fs.writeFileSync(
    path.join(tempDirs.result, "changesHistory.json"),
    JSON.stringify(changesHistory),
    "utf8"
  );
...
function* processChangesBase64(ctx, tempDirs, task, cmd, authorProps, sha256) {
  ...
  let streamObj = yield* streamCreate(ctx, changesDir, indexFile++, {
    highWaterMark: tenStreamWriterBufferSize,
  });
  let curIndexStart = 0;
  let curIndexEnd = Math.min(curIndexStart + tenMaxRequestChanges, forceSaveIndex);
...
function* processUploadToStorage(
  ctx,
  dir,
  storagePath,
  calcChecksum,
  ...
  if (list.length < MAX_OPEN_FILES) {
    yield* processUploadToStorageChunk(
      ctx,
      list,
      dir,
      storagePath,
...
function* processUploadToStorageChunk(
  ctx,
  list,
  dir,
  storagePath,
...
function* processUploadToStorageErrorFile(
  ctx,
  dataConvert,
  tempDirs,
  childRes,
  ...
  yield* processUploadToStorage(
    ctx,
    tempDirs.temp,
    format + "/" + dataConvert.key,
    false,
    tenErrorFiles,
    ignorePrefix
  );
...
function* postProcess(ctx, cmd, dataConvert, tempDirs, childRes, error, isTimeout) {
  ...
  if ((0 !== exitCode && constants.CONVERT_CELLLIMITS !== -exitCode) || null !== exitSignal) {
    ...
    if (-1 !== exitCodesMinorError.indexOf(error)) {
      ...
      yield* processUploadToStorageErrorFile(
        ctx,
        dataConvert,
        tempDirs,
        childRes,
  ...
  if (-1 !== exitCodesUpload.indexOf(error)) {
    if (-1 !== exitCodesCopyOrigin.indexOf(error)) {
      let originPath = path.join(
        path.dirname(dataConvert.fileTo),
        "origin" + path.extname(dataConvert.fileFrom)
      );
...
function* spawnProcess(
  ...
  try {
    ...
    let waitMS = Math.max(
      0,
      task.getVisibilityTimeout() * 1000 - (new Date().getTime() - getTaskTime.getTime())
    );
...
function* ExecuteTask(ctx, task) {
  ...
  if (constants.NO_ERROR === error) {
    ...
    if (canRollback) {
      ...
      let extNew =
        "." + formatChecker.getStringFromFormat(constants.AVS_OFFICESTUDIO_FILE_OTHER_OOXML);
